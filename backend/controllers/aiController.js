...
const suggestPrice = asyncHandler(async (req, res) => {
  ...
  const promptText = `
    ...
    Only reply with a valid JSON format having two properties: "suggestedPrice" (a number, without formatting like commas) and "reasoning" (a short 1-2 sentence explanation). Do not wrap the JSON in markdown blocks like \`\`\`.
  `;

  // Fallback heuristic function if no API key
  const mockAI = () => {
    let base = 500;
  ...
  try {
    ...
    res.json({
      success: true,
      data: {
        suggestedPrice: typeof aiResult.suggestedPrice === 'string' ? parseFloat(aiResult.suggestedPrice) : aiResult.suggestedPrice,
    ...
    // If Gemini fails (rate limit, etc), fall back to mock without crashing
